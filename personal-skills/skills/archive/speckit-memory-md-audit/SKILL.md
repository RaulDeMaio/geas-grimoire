...
description: "Read-only, evidence-based audit of durable and feature memory files ({memory_root}/*.md, {specs_root}/<feature>/memory*.md) to flag stale, contradictory, or low-signal entries. Intentionally expensive — run periodically or when memory quality is in doubt, not on every task."
